        ...
        case 3:
             ...
             && GetMonData(mon, MON_DATA_LEVEL) != MAX_LEVEL)
            {
                ...
                else if (param == LEVEL_CAP)
                {
                    ...
                    const struct Evolution *evolutions = GetSpeciesEvolutions(GetMonData(mon, MON_DATA_SPECIES));
                    ...
                    if (!blockLevelUp)
                        dataUnsigned = gExperienceTables[gSpeciesInfo[GetMonData(mon, MON_DATA_SPECIES)].growthRate][levelUpThreshold];
                }
                else if (param == LEVEL_CAP)
                {
                    u32 levelUpThreshold = GetCurrentLevelCap();
